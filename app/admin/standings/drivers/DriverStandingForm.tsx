...
type DriverStandingValues = z.input<typeof driverStandingSchema>;

export default function DriverStandingForm({ initialData }: { initialData?: DriverStanding | null }) {
  const { drivers, teams, seasons, load } = useReferenceStore();
  const [status, setStatus] = useState<string | null>(null);
  const form = useForm<DriverStandingValues>({
    resolver: zodResolver(driverStandingSchema),
    defaultValues: {
  ...
  const onSubmit = async (values: DriverStandingValues) => {
    setStatus(null);
    const method = initialData ? "PUT" : "POST";
    const endpoint = initialData ? `/api/driver-standings/${initialData.id}` : "/api/driver-standings";
    try {
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-6 md:grid-cols-2">
      ...
      <SelectField
        ...
        name="driver_id"
        register={form.register}
        error={form.formState.errors.driver_id}
